...
const setDbInstance = async () => {
    logger.info("Merchant Portal db");
    try{
    const sequelize = new Sequelize(process.env.MP_DATABASE_NAME, process.env.MP_DATABASE_USERNAME, process.env.MP_DATABASE_PASSWORD, {
    ...
    while (retries < config.maxRetries) {
        try {
            ...
            logger.info("MP Database connected");
            return;
        } catch (err) {
            logger.error("error while connect MP db",err);
            retries++;
        }
    }
    throw new Error(constants.ERROR_MESSAGE.DB_CONNECTION);
}catch(err){
    logger.error("MP error" , err);
    throw err;
}
};
